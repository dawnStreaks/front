...
export const store = new Vuex.Store({
  ...
  actions: {
    ...
    loadSubTenantsdept({commit}, val) {
      this.state.setSubTenant = [];
      Vue.axios.get(`/loadSubTenantsdept/${val}`)
        .then(response => {
          if (response.data.code == 200) {
            //this.state.loading = true,
            var filtered = response.data.tenants;
            for (var i = 0; i < filtered.length; i++) {

              if (filtered[i].name === undefined || filtered[i].id === '') {
            ...
            this.state.setSubTenant = filtered.map(function (item) {
