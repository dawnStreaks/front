            ...
            <b-col cols="12" xl="12" ref="formContainer">
                <transition name="slide">
                    <div class="control-section" style="height:100%;width:100%;background-color: #ffffff;">
                        <b-card>
                            <div slot="header">
                                <b class="h4">{{ get_text('organization_chart@subtenant@l') }}</b>
                                <b-button style="margin-left: 15px;margin-bottom: 5px;margin-top: 5px;"
                                          variant="success" size="md" class="btn float-right" @click="$router.go(-1)">{{
                                    get_text('back@l') }}
                                </b-button>
                            </div>
...
<script>
    ...
    export default Vue.extend({
        ...
        created() {
            this.subtanentsname = this.$route.params.test;
        },
        methods: {
            get_text(textbit) {
